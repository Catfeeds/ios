...
@interface WXDatePickerView()<UIPickerViewDelegate, UIPickerViewDataSource>
...
- (void)configToolView {
    ...
    self.toolView.frame = CGRectMake(0, self.height*0.6, self.frame.size.width, 41);
    self.toolView.backgroundColor = selectedTexColor;
    [self addSubview:self.toolView];
    
    UIButton *saveBtn = [[UIButton alloc] init];
    [saveBtn setTitle:@"确定" forState:UIControlStateNormal];
    saveBtn.titleLabel.font = kFont(14);
    saveBtn.backgroundColor = selectedTexColor;
    [saveBtn setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
    [saveBtn addTarget:self action:@selector(saveBtnClick) forControlEvents:UIControlEventTouchUpInside];
    [self.toolView addSubview:saveBtn];
    [saveBtn mas_makeConstraints:^(MASConstraintMaker *make) {
        make.centerY.right.equalTo(self.toolView);
        make.height.equalTo(@40);
        make.width.equalTo(self.toolView.mas_width).multipliedBy(0.5);
    }];
    ...
    [cancelBtn setTitleColor:selectedTexColor forState:UIControlStateNormal];
    [cancelBtn addTarget:self action:@selector(cancelBtnClick) forControlEvents:UIControlEventTouchUpInside];
    [self.toolView addSubview:cancelBtn];
    [cancelBtn mas_makeConstraints:^(MASConstraintMaker *make) {
        make.centerY.left.equalTo(self.toolView);
        make.height.equalTo(@40);
        make.width.equalTo(self.toolView.mas_width).multipliedBy(0.5);
    }];
}
...
- (void)configPickerView {
    self.pickerView = [[UIPickerView alloc] initWithFrame:CGRectMake(0, self.height*0.6+41, self.frame.size.width, self.height*0.4 - 41)];
